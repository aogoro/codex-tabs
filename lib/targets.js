...
// route-home-kind has to land in the pathname test the RouteScope resolver
// consults, and nowhere else. Searching the whole file for `===`/Codex`` is
// not enough: the bundle holds several `X===`/`||` of its own (26.908 has one
// in a filesystem path helper), and a patch that lands in one of them leaves
// every marker check green while the tab resolves to `other` and the home
// screen crashes on mount.

// Up to 26.825 the test was inline in the resolver, next to the sibling
// pathnames — close enough to assert placement by proximity.
const ROUTE_HOME_KIND_INLINE = /===`\/Codex`\|\|[^;]{0,200}routeKind:`home`/;

// 26.908 pulled it out into a standalone predicate — `function uh(e){return
// e===`/`||!1}` — that the resolver and the telemetry mapper both call. The
// splice belongs there, but `routeKind:`home`` then sits ~186 KB away, so
// proximity proves nothing. Resolve the predicate by name out of the home
// branch instead and work on its declaration.
...
function routeHomeKindApplied(route) {
    ...
    if (ROUTE_HOME_KIND_INLINE.test(route)) return true;
